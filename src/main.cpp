    ...
    switch (st) {
      ...
      case NState::ST_AT_PROBE: {
        ...
        for (int i=0; i<5 && millis()-t0<3000 && !ok; ++i) {
          SerialAT.print("AT\r");
          delay(200);
          String buf; while (SerialAT.available()) buf += (char)SerialAT.read();
...
void loop() {
  ...
  if (millis() - t_btn >= 10) {
    ...
    if (auto e = btn1_get(); e.shortPress) { g_screen = UiScreen::HOME; LOG("[Owl] HOME"); }
    else if (e.longPress) { next_screen(); LOG("[Owl] Cambiar pantalla"); }

    if (auto e2 = btn2_get(); e2.shortPress) { g_screen = UiScreen::MESSAGES; LOG("[Owl] MESSAGES"); }
    else if (e2.longPress) { save_poi(); }
